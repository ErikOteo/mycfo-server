...
public class ConciliacionService {
    ...
    /**
     * Obtiene todos los movimientos sin conciliar (sin paginación) - para
     * compatibilidad
     */
    @Transactional(readOnly = true)
    ...
    /**
     * Obtiene todos los movimientos (conciliados y sin conciliar) - para
     * compatibilidad
     */
    @Transactional(readOnly = true)
    ...
    public List<DocumentoSugeridoDTO> sugerirDocumentos(Long movimientoId, Long empresaId, TipoMoneda monedaFiltro) {
        ...
        // PASO 1: Filtrar documentos candidatos por tipo Y (monto O fecha O
        // descripción)
        ...
        for (DocumentoSugeridoDTO sug : sugerencias) {
            System.out.println("  " + sug.getTipoDocumento() + " #" + sug.getNumeroDocumento() + " - Score: "
                    + sug.getScoreCoincidencia());
        }
    ...
    /**
     * Filtra documentos candidatos que coincidan en tipo Y (monto O fecha O
     * descripción O todos)
     * Esto reduce significativamente el espacio de búsqueda antes de aplicar
     * scoring
     * Carga EAGER de todos los datos para evitar lazy loading
     */
    private List<DocumentoComercial> filtrarDocumentosCandidatos(Movimiento movimiento, Long empresaId,
            TipoMoneda monedaMovimiento) {
        ...
        // Salvaguarda: si por alguna razón el repository no devuelve por moneda,
        // filtrar manualmente
        ...
        if (facturas != null && !facturas.isEmpty()) {
            ...
            for (Factura factura : facturas) {
                try {
                    ...
                    System.err.println(
                            "Error procesando factura ID: " + factura.getIdDocumento() + " - " + e.getMessage());
                    e.printStackTrace();
                }
            }
        ...
        if (pagares != null && !pagares.isEmpty()) {
            for (Pagare pagare : pagares) {
                try {
                    ...
                    System.err
                            .println("Error procesando pagaré ID: " + pagare.getIdDocumento() + " - " + e.getMessage());
                }
            }
        }
        ...
        if (recibos != null && !recibos.isEmpty()) {
            for (Recibo recibo : recibos) {
                try {
                    ...
                    System.err
                            .println("Error procesando recibo ID: " + recibo.getIdDocumento() + " - " + e.getMessage());
                }
            }
        }
     ...
     */
    private boolean cumpleCriteriosFiltrado(Movimiento movimiento, DocumentoComercial documento,
    ...
    private boolean verificarTipoCoincide(DocumentoComercial documento, TipoMovimiento tipoMovimiento) {
        ...
        if (documento instanceof Factura) {
            ...
            // Simplificación: considerar todas las facturas (ajustar según lógica de
            // negocio)
            return true;
        }
    ...
    /**
     * Verifica si el monto es similar (dentro del 15% de diferencia en valor
     * absoluto)
     */
    ...
    /**
     * Calcula el score de coincidencia de fecha considerando días hábiles y fin de
     * semana
     */
    private int calcularScoreFecha(java.time.LocalDateTime fechaMovimiento, java.time.LocalDateTime fechaDocumento) {
        ...
        // 2. Calcular días hábiles de diferencia (más preciso para transacciones
        // bancarias)
    ...
    private List<String> obtenerNombresDelDocumento(DocumentoComercial documento) {
        ...
        if (documento instanceof Factura) {
            Factura factura = (Factura) documento;
            if (factura.getVendedorNombre() != null)
                nombres.add(factura.getVendedorNombre());
            if (factura.getCompradorNombre() != null)
                nombres.add(factura.getCompradorNombre());
        } else if (documento instanceof Pagare) {
            Pagare pagare = (Pagare) documento;
            if (pagare.getBeneficiarioNombre() != null)
                nombres.add(pagare.getBeneficiarioNombre());
            if (pagare.getDeudorNombre() != null)
                nombres.add(pagare.getDeudorNombre());
        } else if (documento instanceof Recibo) {
            Recibo recibo = (Recibo) documento;
            if (recibo.getReceptorNombre() != null)
                nombres.add(recibo.getReceptorNombre());
            if (recibo.getEmisorNombre() != null)
                nombres.add(recibo.getEmisorNombre());
        }
    ...
    private DocumentoComercial buscarDocumento(Long documentoId, Long empresaId) {
        ...
        return facturaRepository.findById(documentoId)
                ...
                .orElseGet(() -> pagareRepository.findById(documentoId)
                        .filter(p -> empresaId.equals(p.getOrganizacionId()))
                        .<DocumentoComercial>map(p -> p)
                        .orElseGet(() -> reciboRepository.findById(documentoId)
    ...
    private DocumentoSugeridoDTO crearSugerenciaDesdeFactura(Factura factura, int score) {
        ...
        dto.setNombreRelacionado(
                factura.getVendedorNombre() != null ? factura.getVendedorNombre() : factura.getCompradorNombre());
    ...
    private DocumentoSugeridoDTO crearSugerenciaDesdePagare(Pagare pagare, int score) {
        ...
        dto.setNombreRelacionado(
                pagare.getBeneficiarioNombre() != null ? pagare.getBeneficiarioNombre() : pagare.getDeudorNombre());
    ...
    private DocumentoSugeridoDTO crearSugerenciaDesdeRecibo(Recibo recibo, int score) {
        ...
        dto.setNombreRelacionado(
                recibo.getReceptorNombre() != null ? recibo.getReceptorNombre() : recibo.getEmisorNombre());
    ...
    /**
     * Actualiza el estado de pago de una factura basado en los movimientos
     * conciliados
     */
    ...
    /**
     * Obtiene estadísticas de conciliación de forma optimizada
     */
    @Transactional(readOnly = true)
    public java.util.Map<String, Object> obtenerResumenEstadisticas(Long empresaId, TipoMoneda moneda) {
        long total;
        long sinConciliar;
        long conciliados;

        if (moneda != null) {
            total = movimientoRepository.countByOrganizacionIdAndMoneda(empresaId, moneda);
            sinConciliar = movimientoRepository.countByOrganizacionIdAndDocumentoComercialIsNullAndMoneda(empresaId,
                    moneda);
            conciliados = movimientoRepository.countByOrganizacionIdAndDocumentoComercialIsNotNullAndMoneda(empresaId,
                    moneda);
        } else {
            total = movimientoRepository.countByOrganizacionId(empresaId);
            sinConciliar = movimientoRepository.countByOrganizacionIdAndDocumentoComercialIsNull(empresaId);
            conciliados = movimientoRepository.countByOrganizacionIdAndDocumentoComercialIsNotNull(empresaId);
        }
        ...
        stats.put("porcentajeConciliado", total == 0 ? 0 : (conciliados * 100.0 / total));
