...
@Slf4j
public class VertexImageScannerService {
    ...
    public ImageScanResult scanImage(byte[] imageBytes, String mimeType, ScanType scanType) {
        long totalStart = System.nanoTime();
        if (imageBytes == null || imageBytes.length == 0) {
            throw new ResponseStatusException(INTERNAL_SERVER_ERROR, "No se recibio contenido de imagen.");
        }
    ...
    private List<String> buildWarnings(Map<String, Object> campos, Set<String> required) {
        ...
        for (String campo : required) {
            ...
            if (valor == null || (valor instanceof String str && str.isBlank())) {
                warnings.add("No se detecto el campo \"" + campo + "\" en la imagen.");
            }
        }
        return warnings;
    }
    ...
    private String buildPrompt(ScanType type) {
        return switch (type) {
            ...
            case INGRESO -> """
                ...
                Analiza la imagen y devolve exclusivamente los datos con esta estructura JSON:
    ...
    public record ImageScanResult(String rawText, Map<String, Object> campos, List<String> warnings) {}
}
