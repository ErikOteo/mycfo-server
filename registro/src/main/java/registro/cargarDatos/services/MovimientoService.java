...
public class MovimientoService {
         ...
         */
        @CacheEvict(value = "movimientosPresupuesto", allEntries = true)
        ...
        public ResumenMensualResponse obtenerResumenMensual(Long organizacionId, String usuarioId,
                        LocalDate fechaReferencia) {
                return obtenerResumenMensual(organizacionId, usuarioId, fechaReferencia, null);
        }

        public ResumenMensualResponse obtenerResumenMensual(Long organizacionId, String usuarioId,
                        LocalDate fechaReferencia, TipoMoneda moneda) {
                if (organizacionId == null && (usuarioId == null || usuarioId.isBlank())) {
                        throw new IllegalArgumentException(
                                        "Se requiere Organizacion o usuario para calcular el resumen mensual");
                }
                ...
                return ResumenMensualResponse.builder()
                                ...
                                // Como los egresos ya son negativos, el resultado neto debe ser la suma
                                // algebraica
        ...
        public MontosMensualesResponse obtenerIngresosMensuales(
                        ...
                        int meses,
                        TipoMoneda moneda) {
                // Para el dashboard queremos la serie a nivel empresa, no por usuario
                // individual
        ...
        private MontosMensualesResponse obtenerMontosMensuales(
                ...
                // usuarioId puede ser null para que el query agregue todos los usuarios de esa
                // empresa.
                if (organizacionId == null) {
                        throw new IllegalArgumentException(
                                        "Organizacion es obligatoria para el resumen mensual por periodo");
                }
        ...
        public ConciliacionResumenResponse obtenerResumenConciliacion(
                ...
                if (organizacionId == null && (usuarioId == null || usuarioId.isBlank())) {
                        throw new IllegalArgumentException(
                                        "Organizacion o usuario son requeridos para el resumen de conciliacion");
                }
                ...
                List<ConciliacionTipoResumen> porTipo = porTipoRaw.stream()
                                .map(item -> {
                                        ...
                                        Double montoConciliado = item[4] != null ? ((Number) item[4]).doubleValue()
                                                        : 0.0;
                                        Double montoPendiente = item[5] != null ? ((Number) item[5]).doubleValue()
                                                        : 0.0;

                                        // Calcular porcentaje
                                        double porcentajeTipo = totalTipo > 0 ? (conciliadosTipo * 100.0) / totalTipo
                                                        : 0.0;
                                        ...
                                        if (tipo == TipoMovimiento.Egreso) {
                                                ...
                                                montoConciliado = montoConciliado != null ? -Math.abs(montoConciliado)
                                                                : 0.0;
                                                montoPendiente = montoPendiente != null ? -Math.abs(montoPendiente)
                                                                : 0.0;
                                        }
        ...
        public MovimientosPresupuestoResponse obtenerMovimientosParaPresupuesto(
                ...
                for (YearMonth mes = inicio; !mes.isAfter(fin); mes = mes.plusMonths(1)) {
                        ...
                        MovimientosPresupuestoResponse.MovimientosMensuales datosMes = procesarMovimientosMes(mesKey,
                                        movimientosMes);
        ...
        private MovimientosPresupuestoResponse.MovimientosMensuales procesarMovimientosMes(
                ...
                BigDecimal totalIngresos = ingresos.stream()
                                .map(m -> BigDecimal
                                                .valueOf(Math.abs(m.getMontoTotal() != null ? m.getMontoTotal() : 0.0)))
                ...
                List<MovimientosPresupuestoResponse.MovimientoPorCategoria> ingresosPorCategoria = agruparPorCategoria(
                                ingresos, totalIngresos);

                List<MovimientosPresupuestoResponse.MovimientoPorCategoria> egresosPorCategoria = agruparPorCategoria(
        ...
        private List<MovimientosPresupuestoResponse.MovimientoPorCategoria> agruparPorCategoria(
                ...
                Map<String, List<Movimiento>> porCategoria = movimientos.stream()
                                .collect(Collectors.groupingBy(
                                                m -> m.getCategoria() != null ? m.getCategoria() : "Sin categoría"));
                ...
                return porCategoria.entrySet().stream()
                                .map(entry -> {
                                        ...
                                        BigDecimal montoCategoria = movimientosCategoria.stream()
                                                        .map(m -> BigDecimal.valueOf(Math.abs(
                                                                        m.getMontoTotal() != null ? m.getMontoTotal()
                                                                                        : 0.0)))
                                                        .reduce(BigDecimal.ZERO, BigDecimal::add);

                                        BigDecimal porcentaje = totalTipo.compareTo(BigDecimal.ZERO) > 0
                                                        ? montoCategoria.multiply(BigDecimal.valueOf(100))
                                                                        .divide(totalTipo, 2, RoundingMode.HALF_UP)
                                                        : BigDecimal.ZERO;
